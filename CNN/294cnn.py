...
class CNNClassifier(nn.Module):
    def __init__(self,
        ...
        ## de notebook cnn
        self.convs = []
        for filter_lenght in args.FILTERS_LENGTH:
            self.convs.append(
                nn.Conv1d(vector_size, args.FILTERS_COUNT, filter_lenght)
            )
        self.convs = nn.ModuleList(self.convs)
        self.fc = nn.Linear(args.FILTERS_COUNT * len(args.FILTERS_LENGTH), 128)
    ...
    def forward(self, x):
        ...
        x = torch.cat(x, dim=1)
        x = F.relu(self.fc(x))
        x = self.output(x) #para multiclase sin sigmoid
        return x
